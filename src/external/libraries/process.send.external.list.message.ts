...
export class ProcessSendExternalListMessage {
  protected body: ExternalListMessagePayloadDto;
  ...
  public async send(body: ExternalListMessagePayloadDto, send_via_queue?: boolean) {
    this.body = body;
  ...
  private async setPayload() {
    this.payload = await this.externalMessageService.getListMessagePayload(this.body);
  }
